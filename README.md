...
```python
# ネットワーク設定
NUM_RES_BLOCKS = 6          # Residualブロック数（5x5盤面に最適化）
NUM_FILTERS = 64            # フィルタ数
...
# MCTS設定（プレイ時）
play_mcts_config.NUM_SIMULATIONS = 200  # より多いシミュレーション
play_mcts_config.C_PUCT = 1.5          # より探索重視
play_mcts_config.DIRICHLET_EPSILON = 0.0  # ノイズなし（決定的）

# 学習設定
BATCH_SIZE = 1024          # バッチサイズ
LEARNING_RATE = 0.001      # 学習率（0.2も使用可能、ウォームアップ必要）
USE_WARMUP = False         # 高い学習率用のウォームアップ
WARMUP_STEPS = 1000        # ウォームアップステップ数
MAX_GRAD_NORM = 10.0       # 勾配クリッピング閾値
...
詳細な最適化ガイドは [OPTIMIZATION_GUIDE.md](OPTIMIZATION_GUIDE.md) を参照してください。
...
入力: (66, 5, 5) テンソル
  ...
  ├─ 32-39チャンネル: 自分の黒タイル数（履歴8手分）
  ├─ 40-47チャンネル: 自分のグレータイル数（履歴8手分）
  ├─ 48-55チャンネル: 相手の黒タイル数（履歴8手分）
  ├─ 56-63チャンネル: 相手のグレータイル数（履歴8手分）
  ├─ 64チャンネル: 現在のプレイヤーID
  └─ 65チャンネル: 手数（正規化済み）
...
| **MCTS（プレイ時）** | | |
| シミュレーション回数 | 200 | より深い探索で精度向上 |
| C_PUCT | 1.5 | より探索を重視 |
| ディリクレε | 0.0 | ノイズなし（決定的な選択） |
| **評価** | | |
| 評価間隔 | 1000ステップ | 学習中の評価頻度 |
...
### 最適化内容（v2.0）
1. **ネットワーク最適化**: 5x5盤面に合わせたモデルサイズ
   - Residualブロック: 8 → 6（約20%高速化）
   - パラメータ数: ~1.3M → ~1.0M（メモリ効率向上）
   
2. **プレイ時MCTS最適化**: 学習と対戦で別設定
   - シミュレーション回数: 50 → 200（対戦時）
   - C_PUCT: 1.0 → 1.5（より探索重視）
   - ノイズなし（決定的な選択）
   
3. **高学習率サポート**: LR 0.2での学習を可能に
   - 学習率ウォームアップスケジューラ
   - 勾配クリッピング（最大ノルム: 10.0）
   - 安定した学習を実現
